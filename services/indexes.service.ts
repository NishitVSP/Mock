...
getIndexLastPrices().then((prices) => {
    for (const [indexName, lastPrice] of Object.entries(prices)) {
        console.log(`${indexName}: ${lastPrice}`);
    }
}).catch((error) => {
    console.error('Error fetching index last prices:', error);
});